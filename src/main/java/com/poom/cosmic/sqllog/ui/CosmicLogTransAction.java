 ...
 * @author kun
 */
public class CosmicLogTransAction extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        if (!currentCaret.hasSelection() || selectedText == null || selectedText.length() == 0) {
            NotificationUtils.showMessage(project, "请先用光标选定cosmic的sql日志和参数日志");
            return;
        }
        ...
        if (strNum != 2) {
            String message;
            if (strNum < 2) {
               message = "光标选定cosmic的sql日志和参数日志时，尽量包含“Preparing: ”和“Parameters: ” ";
            } else {
